...
using ZoroKit.Core.Interfaces.Providers;
using ZoroKit.Core.Interfaces.Services;
using ZoroKit.Infrastructure.Configuration;
using ZoroKit.Infrastructure.FileSystem;
using ZoroKit.Infrastructure.Logging;
using ZoroKit.Infrastructure.Network;
using ZoroKit.Infrastructure.Privilege;
using ZoroKit.Infrastructure.Process;
using ZoroKit.Infrastructure.Runtime;
using ZoroKit.Infrastructure.VersionProviders;
using ZoroKit.UI.Services;
using ZoroKit.UI.ViewModels;
using ZoroKit.UI.Views;

namespace ZoroKit.UI;
...
public partial class App : System.Windows.Application
{
    ...
    protected override async void OnStartup(StartupEventArgs e)
    {
        ...
        _singleInstanceMutex = new Mutex(true, "ZoroKit_SingleInstance_E8F3A2B1", out var isNewInstance);
        if (!isNewInstance)
        {
    ...
    private async Task CheckForAppUpdateAsync()
    {
        try
        {
            ...
            {
                Dispatcher.Invoke(() =>
                    toastService.ShowInfo($"ZoroKit v{latestTag} mevcut! Güncellemeler sayfasını kontrol edin."));
            }
        }
        ...
        menu.Items.Add(new SWF.ToolStripMenuItem("Terminal", CreateTerminalIcon(),
            async (_, _) =>
            {
                try
                {
                    ...
                    {
                        FileName = "cmd.exe",
                        Arguments = "/K \"color 0A && title ZoroKit Terminal\"",
    ...
    private void UpdateTrayIconStatus(IServiceController apache, IServiceController mariaDb)
    {
        ...
        if (apacheRunning && mariaRunning)
        {
            ...
            statusText = "ZoroKit - Apache: \u00c7al\u0131\u015f\u0131yor, MariaDB: \u00c7al\u0131\u015f\u0131yor";
        }
        else if (!apacheRunning && !mariaRunning)
        {
            dotColor = SD.Color.FromArgb(255, 244, 67, 54);
            statusText = "ZoroKit - Apache: Durdu, MariaDB: Durdu";
        }
        else
        {
            ...
            statusText = $"ZoroKit - Apache: {aText}, MariaDB: {mText}";
        }
    ...
    private static void ConfigureServices(IServiceCollection services, string basePath)
    {
        ...
        services.AddSingleton(sp =>
        {
            ...
            client.DefaultRequestHeaders.UserAgent.ParseAdd("Mozilla/5.0 (Windows NT 10.0; Win64; x64) ZoroKit/1.0");
        ...
        {
            client.DefaultRequestHeaders.TryAddWithoutValidation("User-Agent", "ZoroKit/1.0 (Windows; https://github.com/yenifont/ZoroKit)");
    ...
    private static string? FindSolutionRoot(string startPath)
    {
        ...
        while (dir != null)
        {
            if (File.Exists(Path.Combine(dir.FullName, "ZoroKit.sln")) ||
                File.Exists(Path.Combine(dir.FullName, "ZoroKit.slnx")))
                return dir.FullName;
            dir = dir.Parent;
        }
        return null;
    ...
    private static readonly string InstallPathFile = Path.Combine(
        ...
        "ZoroKit", "installpath.txt");

    /// <summary>
    /// 1) Dev mode: walk up to ZoroKit.sln
