; ZoroKit Installer Script - Inno Setup 6+
; KULLANICI MUTLAKA "ZoroKit-Setup-xxx.exe" CALISTIRMALI. Sadece ZoroKit.exe calistirilirsa kisayol ve Program Ekle Kaldir OLUSMAZ.
; Bu script ile: C:\ZoroKit\ZoroKit.exe + Masaustu kisayolu + Baslat menusu (ZoroKit + ZoroKit Kaldir) + Program Ekle/Kaldir kaydi.
...
; Output
OutputDir=Output
OutputBaseFilename=ZoroKit-Setup-{#MyAppVersion}

; Icons & branding
SetupIconFile=..\src\ZoroKit.UI\Resources\app.ico
...
[Messages]
; Hosgeldin, hedef klasor, guzel URL + otomatik sanal konak, hazir, tamamlandi
WelcomeLabel1={#MyAppName} Kurulumuna Hosgeldiniz
WelcomeLabel2=Bu sihirbaz {#MyAppName} {#MyAppVersion} surumunu bilgisayariniza kuracaktir.%n%n{#MyAppName}, Apache, PHP ve MariaDB ile yerel web gelistirme ortaminizi tek yerden yonetir.%n%nProjeleriniz guzel URL alacak: https://zorokit.app%nOtomatik sanal konaklar desteklenir.%n%nDevam etmek icin Ileri'ye tiklayin.
FinishedHeadingLabel={#MyAppName} Kurulumu Tamamlandi!
FinishedLabel={#MyAppName} kuruldu.%n%nYerel adres: https://zorokit.app (ilk acilista otomatik eklenir).%n%nCALISTIRMA: Masaustundeki veya Baslat menusundeki "ZoroKit" kisayoluna tiklayin.%n%nKALDIRMA: Ayarlar > Uygulamalar > {#MyAppName} > Kaldir
...
ReadyLabel2a={#MyAppName} kurulmaya hazir.%n%nHedef: C:\{#MyAppName}%nYerel adres: https://zorokit.app%n%nBaslamak icin Kur'a tiklayin.
...
[Icons]
; C:\ZoroKit\ZoroKit.exe kurulur. Asagidaki kisayollar HER ZAMAN olusturulur (Tasks yok = zorunlu).
; Baslat menusu: ZoroKit + ZoroKit Kaldir
...
// Kill all ZoroKit-related processes
...
// Remove ZoroKit-managed block from hosts file (between START/END markers)
...
begin
  ...
  begin
    Line := Lines[I];
    // Detect start of ZoroKit block
    if Pos('# --- ZoroKit START ---', Line) > 0 then
    begin
      InBlock := True;
      Continue;
    end;
    // Detect end of ZoroKit block
    if Pos('# --- ZoroKit END ---', Line) > 0 then
    begin
      InBlock := False;
      Continue;
    end;
...
begin
  ...
  // All are fine — no restart required since ZoroKit hasn't started yet
end;
...
begin
  ...
  begin
    ...
    if DirExists(AppDir) then
      Exec('cmd.exe', '/C rd /S /Q "' + AppDir + '" 2>nul & if exist "' + AppDir + '" (echo @rd /S /Q "' + AppDir + '" > "%TEMP%\ZoroKit_cleanup.cmd" & reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\RunOnce" /v ZoroKitCleanup /t REG_SZ /d "%TEMP%\ZoroKit_cleanup.cmd" /f)', '', SW_HIDE, ewWaitUntilTerminated, ResultCode);
  end;
end;
